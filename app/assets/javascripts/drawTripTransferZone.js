...
function makeTransferZone(transferZoneData){    
    ...
    if (transferZoneData !== undefined) { //fix this bug, why when you remove some transferzones transferZoneData["event_ids"] in undefined?
      ...
      if(transferZoneData["event_ids"].length == 2){
        ...
        form = "<h5>Please select a transportation before you delete this trasfer zone! </h5>"
        ...
        form = "<h5>This is the first/last transferzone, so it is not allowed to delete it.</h>";
      } else {
        form = "error";
      }
      ...
      marker.addListener('dragend', function(e) {
        var r = confirm("Are you sure to change the transferzone to the nearest intermediate point of current position?");
        if (r==true)
          {
